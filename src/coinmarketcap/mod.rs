...
pub fn cmc_main() {
    ...
    //     let _date = chrono::Utc::now().format("%Y-%m-%d");
    //     let _start_date = start_date.clone();
    //     let _end_date = end_date.clone();
    //     let id = coindata.id.unwrap();
    //     let rank = coindata.rank.unwrap();
    //     println!("Requesting: {:?} - {:?}", &rank, &id);
    //
    //     pool.execute(move || {
    //         sleep(Duration::from_millis(500));
    //         let csv_data = download_data(&id, _start_date, _end_date);
    //         let fp = format!("./data/{}/{}_{}.csv", &_date, rank, id);
    //         let _ = std::fs::write(&fp, csv_data.join("\n"));
    //     });
    // }
    // pool.join();
